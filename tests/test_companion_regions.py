...
from repeater.companion.bridge import RepeaterCompanionBridge, normalize_region_name
...
@pytest.mark.asyncio
async def test_scoped_channel_preserves_full_secret_history_and_echo_tracking():
    packets = []
    ...
    assert len(events) == 1
    assert events[0].packet_hash == packet.calculate_packet_hash().hex().upper()
    assert events[0].text == "hello"
    assert events[0].timestamp == 123
